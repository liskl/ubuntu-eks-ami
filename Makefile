...
PACKER_VARIABLES := ami_name binary_bucket_name kubernetes_version kubernetes_build_date docker_version cni_version cni_plugin_version source_ami_id arch instance_type vpc_id, subnet_id
AWS_DEFAULT_REGION ?= us-east-2

K8S_VERSION_PARTS := $(subst ., ,$(kubernetes_version))
K8S_VERSION_MINOR := $(word 1,${K8S_VERSION_PARTS}).$(word 2,${K8S_VERSION_PARTS})

ami_name ?= ubuntu-eks-node-$(K8S_VERSION_MINOR)-v$(shell date +'%Y%m%d')
vpc_id ?= vpc-0318e96d9d4a91830
subnet_id ?= subnet-09e3ae56a1d5b69f5
arch ?= x86_64
ifeq ($(arch), arm64)
